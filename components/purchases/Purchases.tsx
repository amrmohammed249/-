...
const SmartPurchase: React.FC<SmartPurchaseProps> = ({ windowId }) => {
    const { suppliers, inventory, addPurchase, showToast, sequences, scannedItem } = useContext(DataContext);
    ...
    const handleFinalize = useCallback(async (paymentStatus: Purchase['status'], print: boolean = false) => {
        ...
        const newPurchaseData: Omit<Purchase, 'id' | 'journalEntryId'> = {
        ...
        try {
            const createdPurchase = addPurchase(newPurchaseData);
    ...
    }, [items, supplier, activeBill.date, totals, addPurchase, showToast, resetBill]);
        ...
        <div className="flex flex-col h-full bg-[--bg] text-[--text] font-sans">
            ...
            <header className="flex-shrink-0 bg-[--panel] dark:bg-gray-800 shadow-sm p-3 flex flex-wrap justify-between items-center gap-4">
                <div className="flex items-center gap-4">
                    <h1 className="text-xl font-bold text-gray-800 dark:text-gray-100">فاتورة مشتريات</h1>
                    ...
                    <input 
                      ...
                      onChange={e => {setActiveBill(p => ({...p, date: e.target.value})); setIsDirty(true);}}
                ...
                <div className="flex-grow max-w-sm relative">
                    ...
                    {supplierSearchResults.length > 0 && (
                        <div className="absolute top-full right-0 left-0 bg-[--panel] dark:bg-gray-800 shadow-lg rounded-b-lg border dark:border-gray-700 z-30">
                            {supplierSearchResults.map((s, index) => (
                                <div key={s.id} 
                                    onClick={() => { setSupplier(s); setSupplierSearchTerm(''); productSearchRef.current?.focus(); setIsDirty(true);}} 
                ...
                <aside className="w-full md:w-1/3 flex flex-col gap-4">
                    ...
                    <div className="bg-[--panel] dark:bg-gray-800 rounded-lg shadow-[--shadow] p-4 space-y-3">
                        <div className="grid grid-cols-2 gap-2">
                             <button onClick={() => handleFinalize('مدفوعة', false)} disabled={isProcessing} className="flex items-center justify-center gap-2 text-lg font-bold p-3 bg-[--accent-2] text-white rounded-lg hover:opacity-90 disabled:opacity-50">
                                <BanknotesIcon className="w-6 h-6"/> مدفوعة
                             </button>
                             <button onClick={() => handleFinalize('مستحقة', false)} disabled={isProcessing} className="flex items-center justify-center gap-2 text-lg font-bold p-3 bg-[--warning] text-white rounded-lg hover:opacity-90 disabled:opacity-50">
                                <UsersIcon className="w-6 h-6"/> مستحقة
                             </button>
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-2">
                            ...
                            <button onClick={resetBill} className="w-full text-md p-2 bg-gray-200 dark:bg-gray-700 text-gray-800 dark:text-gray-200 rounded-lg hover:bg-gray-300 dark:hover:bg-gray-600">
                                فاتورة جديدة (F3)
                            </button>
                        </div>
                    </div>
            ...
            {isAddSupplierModalOpen && (
                <Modal isOpen={isAddSupplierModalOpen} onClose={() => setAddSupplierModalOpen(false)} title="إضافة مورد جديد">
                    <AddSupplierForm onClose={() => setAddSupplierModalOpen(false)} onSuccess={(s) => { setSupplier(s); setAddSupplierModalOpen(false); setIsDirty(true); }} />
                </Modal>
            )}
            {purchaseToView && (
