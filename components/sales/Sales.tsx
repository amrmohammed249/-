...
interface SmartSaleProps {
  windowId: string;
}

const SmartSale: React.FC<SmartSaleProps> = ({ windowId }) => {
    const { customers, inventory, addSale, showToast, sequences, scannedItem } = useContext(DataContext);
    ...
    // Effect to handle global barcode scans
    useEffect(() => {
        // Only process scan if this window is visible and the scan is new
    ...
    const handleFinalize = useCallback(async (paymentStatus: Sale['status'], print: boolean = false) => {
        ...
        const newSaleData: Omit<Sale, 'id' | 'journalEntryId'> = {
        ...
        try {
            const createdSale = addSale(newSaleData);
    ...
    }, [items, customer, activeInvoice.date, totals, addSale, showToast, resetInvoice]);
    ...
    useEffect(() => {
        const handleKeyDown = (e: KeyboardEvent) => {
            ...
            if (e.key === 'F9') {
                e.preventDefault();
                handleFinalize('مدفوعة', true);
            }
            if (e.key === 'F2') {
                e.preventDefault();
                handleFinalize('مستحقة', false);
            }
            if (e.key === 'F3') {
                e.preventDefault();
                resetInvoice();
        ...
        setItems(currentItems => {
            ...
            if (field === 'unitId') {
                ...
                if (field === 'price' && numValue === 0) {
                    showToast('تجاوز السعر 0 يتطلب صلاحية مدير.', 'warning');
                }

                if (field === 'quantity') {
                     let quantityInBaseUnit = numValue;
                     if (item.unitId !== 'base') {
                         const packingUnit = inventoryItem.units.find((u: PackingUnit) => u.id === item.unitId);
                         if (packingUnit && packingUnit.factor > 0) {
                             quantityInBaseUnit = numValue * packingUnit.factor;
                         }
                     }
                     if (inventoryItem && quantityInBaseUnit > inventoryItem.stock) {
                         showToast(`الكمية المطلوبة (${numValue} ${item.unitName}) تتجاوز المخزون المتاح (${inventoryItem.stock} ${inventoryItem.baseUnit}).`, 'warning');
                     }
                }
    ...
    const handleItemInputKeyDown = (e: React.KeyboardEvent, itemId: string, field: 'quantity' | 'unit' | 'price') => {
        if (e.key === 'Tab' && !e.shiftKey) {
            if (field === 'quantity') {
                e.preventDefault();
                itemInputRefs.current[itemId]?.unit?.focus();
            } else if (field === 'unit') {
                e.preventDefault();
                itemInputRefs.current[itemId]?.price?.focus();
            } else if (field === 'price') {
                e.preventDefault();
                productSearchRef.current?.focus();
            }
        ...
        <div className="flex flex-col h-full bg-[--bg] text-[--text] font-sans">
            ...
            <header className="flex-shrink-0 bg-[--panel] dark:bg-gray-800 shadow-sm p-3 flex flex-wrap justify-between items-center gap-4">
                <div className="flex items-center gap-4">
                    <h1 className="text-xl font-bold text-gray-800 dark:text-gray-100">فاتورة مبيعات</h1>
                    ...
                    <input 
                      ...
                      onChange={e => { setActiveInvoice(p => ({...p, date: e.target.value})); setIsDirty(true); }}
                                        ...
                                        <tr key={item.itemId + index} className="border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-900/20">
                                            ...
                                            <td className="px-2 py-2">
                                                <input 
                                                    ...
                                                    onKeyDown={(e) => handleItemInputKeyDown(e, item.itemId, 'quantity')}
                                            ...
                                            <td className="px-2 py-2">
                                                <select 
                                                    ...
                                                    onKeyDown={(e) => handleItemInputKeyDown(e, item.itemId, 'unit')}
                                            ...
                                            <td className="px-2 py-2">
                                                <input 
                                                    ...
                                                    onKeyDown={(e) => handleItemInputKeyDown(e, item.itemId, 'price')} 
                                                    className="input-style w-full text-center"/>
                                            </td>
                ...
                <aside className="w-full md:w-1/3 flex flex-col gap-4">
                    ...
                    <div className="bg-[--panel] dark:bg-gray-800 rounded-lg shadow-[--shadow] p-4 space-y-3">
                        <div className="grid grid-cols-2 gap-2">
                             <button onClick={() => handleFinalize('مدفوعة', true)} disabled={isProcessing} className="flex items-center justify-center gap-2 text-lg font-bold p-3 bg-[--accent-2] text-white rounded-lg hover:opacity-90 disabled:opacity-50">
                                <BanknotesIcon className="w-6 h-6"/> نقدي
                             </button>
                             <button onClick={() => handleFinalize('مستحقة', false)} disabled={isProcessing} className="flex items-center justify-center gap-2 text-lg font-bold p-3 bg-[--warning] text-white rounded-lg hover:opacity-90 disabled:opacity-50">
                                <UsersIcon className="w-6 h-6"/> آجل
                             </button>
                        </div>
                        <button onClick={() => handleFinalize('مدفوعة', true)} disabled={isProcessing} className="w-full text-xl font-bold p-4 bg-[--accent] text-white rounded-lg hover:opacity-90 disabled:opacity-50 disabled:cursor-wait">
                            {isProcessing ? 'جاري الحفظ...' : 'حفظ وطباعة (F9)'}
                        </button>
                        <div className="grid grid-cols-2 gap-2">
                             <button onClick={() => handleFinalize('مستحقة', false)} className="w-full text-md p-2 bg-gray-200 dark:bg-gray-700 text-gray-800 dark:text-gray-200 rounded-lg hover:bg-gray-300 dark:hover:bg-gray-600">
                                حفظ كمسودة (F2)
                            </button>
                            <button onClick={resetInvoice} className="w-full text-md p-2 bg-gray-200 dark:bg-gray-700 text-gray-800 dark:text-gray-200 rounded-lg hover:bg-gray-300 dark:hover:bg-gray-600">
                                فاتورة جديدة (F3)
                            </button>
                        </div>
                    </div>
